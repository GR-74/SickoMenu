...
namespace Menu {
    ...
    void SaveMenuSize(const ImVec2& size) {
        std::ofstream configFile("menu_config.cfg");
        if (configFile.is_open()) {
            configFile << size.x << " " << size.y;
            configFile.close();
        }
    }

    ImVec2 LoadMenuSize() {
        std::ifstream configFile("menu_config.cfg");
        ImVec2 size(600, 450); // Default size
        if (configFile.is_open()) {
            configFile >> size.x >> size.y;
            configFile.close();
        }
    ...
    void Init() {
        ImVec2 savedSize = LoadMenuSize();
        ImGui::SetNextWindowSize(savedSize * State.dpiScale, ImGuiCond_None);
